# Requirements Archive: v2.0 Resume Tailoring Flow
...
- [x] **LIBRARY-01**: Resume versions are stored as separate JSON files with a metadata index (`index.json`) in `server/data/resume_library/`
- [x] **LIBRARY-02**: User can list, view, create, rename, and delete resume versions from a Resume Library page
- [x] **LIBRARY-03**: Tailored resumes are auto-named using "Company - Role" format (e.g., "Meridian Software - Fullstack Developer")
- [x] **LIBRARY-04**: Existing `resume.json` is migrated to the library structure on first launch, preserving all data
- [x] **LIBRARY-05**: User can select which resume version to use as the base for analysis and tailoring
...
|-------------|-------|--------|
| LIBRARY-01 | Phase 9 | Complete |
| LIBRARY-02 | Phase 9 | Complete |
| LIBRARY-03 | Phase 12 | Complete |
| LIBRARY-04 | Phase 9 | Complete |
| LIBRARY-05 | Phase 9 | Complete |
...
LIBRARY-01/02/04/05 were checked off at archive time (2026-07-26) though the live `.planning/REQUIREMENTS.md` never had their checkboxes updated after Phase 9 executed. Verified against the codebase before archiving: `server/data/resume_library/index.json`, CRUD routes in `server/index.js`, and `client/src/pages/ResumeLibrary.jsx` all exist and are exercised by later phases (12-15) — this was a stale-doc gap, not an unimplemented requirement.
